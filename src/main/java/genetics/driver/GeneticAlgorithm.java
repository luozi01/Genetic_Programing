...
public class GeneticAlgorithm<T extends Chromosome> {
  ...
  protected GeneticAlgorithm(
      @NonNull final Initializer<T> initializer,
    ...
    if (uniformRate < 0 || uniformRate > 1) {
      throw new IllegalArgumentException(
          String.format("Uniform rate should be [0, 1] but %f found", uniformRate));
    }
    if (mutationRate < 0 || mutationRate > 1) {
      throw new IllegalArgumentException(
          String.format("Mutation rate should be [0, 1] but %f found", mutationRate));
    }
    // population
  ...
  public void evolve(int iteration) throws ExecutionException, InterruptedException {
    if (iteration <= 0) {
      throw new IllegalArgumentException(
          String.format(
              "%d generations is invalid. The number of generations must be > 0.", iteration));
    }
  ...
  protected Population<T> evolvePopulation() throws ExecutionException, InterruptedException {
    ...
    while (nextGeneration.size() < populationSize) {
      ...
      if (RANDOM.nextDouble() < mutationRate) {
        pair =
            Tuples.pair(mutationPolicy.mutate(pair.getOne()), mutationPolicy.mutate(pair.getTwo()));
      }
  ...
  public void updateGlobal(T chromosome) {
    if (bestChromosome.isEmpty() || chromosome.betterThan(bestChromosome.get())) {
