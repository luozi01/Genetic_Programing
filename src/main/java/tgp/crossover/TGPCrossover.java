...
@AllArgsConstructor
public class TGPCrossover implements CrossoverPolicy<TGPChromosome> {
  private final TGPParams manager;

  // Todo simplify

  /**
   * Method that implements the subtree crossover described from Section 2.4 of "A Field Guide to
   * Genetic Programming"
   *
   * @param c1 one tree to be crossover with
   * @param c2 Another tree to be crossover with
   */
  @Override
  public Pair<TGPChromosome, TGPChromosome> crossover(TGPChromosome c1, TGPChromosome c2) {
    ...
    TGPChromosome chromosome1 = c1.makeCopy();
    TGPChromosome chromosome2 = c2.makeCopy();
    ...
    if (iMaxDepth1 <= iMaxDepthForCrossover && iMaxDepth2 <= iMaxDepthForCrossover) {
      ...
      do {
        ...
        if (pCutPoint1 != null && pCutPoint2 != null) {
          Pair<Pair<TreeNode, TreeNode>, Pair<TreeNode, TreeNode>> result =
  ...
  private Pair<Pair<TreeNode, TreeNode>, Pair<TreeNode, TreeNode>> swap(
      Pair<TreeNode, TreeNode> cutPoint1, Pair<TreeNode, TreeNode> cutPoint2) {
