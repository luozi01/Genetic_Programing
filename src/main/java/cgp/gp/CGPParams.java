...
public class CGPParams {
  ...
  public static CGPParams initialiseParameters(
      int numInputs, int numNodes, int numOutputs, int arity) {
    if (numInputs <= 0) {
      throw new IllegalArgumentException(
          String.format(
              "Number of chromosome inputs cannot be less than one; " + "%d is invalid.",
              numInputs));
    }
    ...
    if (numNodes < 0) {
      throw new IllegalArgumentException(
          String.format(
              "Number of chromosome nodes cannot be negative; " + "%d is invalid.\n", numNodes));
    }

    if (numOutputs < 0) {
      throw new IllegalArgumentException(
          String.format(
              "Number of chromosome outputs cannot be less than one; " + "%d is invalid.",
              numOutputs));
    }

    if (arity < 0) {
      throw new IllegalArgumentException(
          String.format("Node arity cannot be less than one; %d is invalid.", arity));
    }
  ...
  public int getRandomFunction(int functionSize) {
    ...
    if (functionSize < 1) {
      throw new IllegalArgumentException(
          "Cannot assign the function gene a value as the Function Set is empty.");
    }
    return random.nextInt(functionSize);
  }

  /** returns a random input for the given node */
  public int getRandomNodeInput(int numInputs, int numNodes, int nodePosition) {
    return random.nextDouble() < recurrentConnectionProbability
        ? random.nextInt(numNodes - nodePosition) + nodePosition + 1
        : /* pick any ahead nodes or the node itself */ random.nextInt(
            numInputs + nodePosition); /* pick any previous node including inputs */
  }

  /**
   * Adds given node function to given function set with given name. Disallows exceeding the
   * function set size.
   */
  ...
  /**
   * used as an interface to adding pre-set node functions. returns one if successful, zero
   * otherwise.
   */
  public void addPresetFunctionToFunctionSet(String functionName) {
    ...
    switch (functionName.toLowerCase(Locale.ROOT)) {
      ...
      default:
        throw new IllegalArgumentException(
            String.format(
                "Warning: function '%s' is not known which can be custom functions.",
                functionName));
    }
  }
  ...
  public String toString() {
    ...
    sb.append(
        String.format(
            "Evolutionary Strategy:\t\t\t(%d%c%d)-ES\n", mu, evolutionaryStrategy, lambda));
    ...
    sb.append(
        String.format("Recurrent Connection Probability:\t%f\n", recurrentConnectionProbability));
