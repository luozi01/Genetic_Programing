...
public final class Node {
  @Getter @Setter private int function;

  @Getter @Setter private boolean active;

  @Getter @Setter private double output;

  @Getter @Setter private int actArity;

  @Getter @Setter private int maxArity;
  ...
  public static Node initializeNode(CGPParams params, int index) {
    ...
    for (int i = 0; i < params.getArity(); i++) {
      node.inputs[i] =
          params.getRandomNodeInput(params.getNumInputs(), params.getNumNodes(), index);
  ...
  public int getInput(int index) {
    if (index < 0 || index >= this.inputs.length) {
      throw new IndexOutOfBoundsException(
          String.format("Expect index range within [0, %s)", this.inputs.length));
    }
  ...
  public double getWeight(int index) {
    if (index < 0 || index >= this.weights.length) {
      throw new IndexOutOfBoundsException(
          String.format("Expect index range within [0, %s)", this.weights.length));
    }
